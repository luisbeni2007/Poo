...
class ManterHorarioUI:
    ...
    def listar():
        ...
        if len(horarios) == 0:
            ...
            for obj in horarios:
                ...
                nome_cliente = cliente.get_nome() if cliente is not None else "N/A"
                descricao_servico = servico.get_descricao() if servico is not None else "N/A"
    ...
    def inserir():
        ...
        cliente = st.selectbox("Informe o cliente", clientes, format_func=lambda c: c.get_nome() if c else "")
        servico = st.selectbox("Informe o serviço", servicos, format_func=lambda s: s.get_descricao() if s else "")
    ...
    def atualizar():
        ...
        if len(horarios) == 0:
            ...
            id_cliente = op.get_id_cliente() if op.get_id_cliente() not in [0, None] else None
            id_servico = op.get_id_servico() if op.get_id_servico() not in [0, None] else None

            cliente_index = next((i for i, c in enumerate(clientes) if c.get_id() == id_cliente), 0)
            servico_index = next((i for i, s in enumerate(servicos) if s.get_id() == id_servico), 0)

            cliente = st.selectbox("Informe o novo cliente", clientes, index=cliente_index, format_func=lambda c: c.get_nome() if c else "")
            servico = st.selectbox("Informe o novo serviço", servicos, index=servico_index, format_func=lambda s: s.get_descricao() if s else "")
